/**
 * by xieyuanyuan 20170808
 * 使用reflux 书写的todoList
 */
...
import TodoStore from '../stores/todoListStore.jsx';

/* 组件包裹 */
const TodoList = createReactClass({
  mixins: [Reflux.listenTo(TodoStore, 'handleChange')],
...
const TypeNew = createReactClass({
  ...
  handleAdd: function (e) {
    ...
    if (name !== '' || age !== '') {
      //只要名称和年龄有一个不为空这时候就可以添加新的一项
      TodoActions.addItem({id: new Date().toTimeString(), name: this.state.name, age: this.state.age});
    }
    //更新输入框中的值
    this.setState({
      name: '',
  ...
  render: function () {
    return (
      <Form layout="inline" onSubmit={this.handleAdd}>
        <FormItem label="姓名">
          <span><Input value={this.state.name} onChange={this.handleChangeName} placeholder="请输入姓名" autoComplete="off" /></span>
        </FormItem>
        <FormItem label="年龄">
          <span><Input value={this.state.age} onChange={this.handleChangeAge} placeholder="请输入年龄" autoComplete="off" /></span>
        </FormItem>
        <FormItem>
